 ...
 * Play a move
 ****************************************************************************/
void ChessRules::PlayMove(Move imove, bool histPush)
{
    if (histPush) {
 ...
 ****************************************************************************/
bool ChessRules::AttackedSquare( Square square, bool enemy_is_white, bool self )
{
    Square dst;
    ...
    while( nbr_rays-- )
    {
        ...
        while( ray_len-- )
        {
            ...
            else
            {
                ...
                else if (self && IsWhite(piece) && !enemy_is_white) {
                    if( to_mask[piece] & mask )
                        return true;
                }

                else if (self && IsBlack(piece) && enemy_is_white) {
                    if( to_mask[piece] & mask )
                        return true;
                }
...
bool ChessRules::Evaluate( MOVELIST *p, TERMINAL &score_terminal ) {
    MOVELIST local_list;
    MOVELIST list = local_list;
    if (p != nullptr) {
        list = *p;
    }
    /* static ;remove for thread safety */ ;
    int i, any;
    Square my_king, enemy_king;
//    for (int j = 0; j < 64; ++j) {
//        if (squares[j] == 'k') {
//            bking_square = Square(j);
//        } else if (squares[j] == 'K') {
//            wking_square = Square(j);
//        }
//    }
    bool okay;
